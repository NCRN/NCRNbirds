...
setGeneric(name="DayXVisit",function(object,points=NA,AOU=NA,years=NA,times=NA,band=1,visits=NA,
...
setMethod(f="DayXVisit", signature=c(object="list"),
          function(object, points, AOU, years, band, visits, output,...) {
            OutMat<-lapply(X=object, FUN=DayXVisit, points=points, AOU=AOU, years=years, times=times,band=band,visits=visits,...)
            switch(output,
                   list= return(OutMat),
...
setMethod(f="DayXVisit", signature=c(object="NCRNbirds"),
          function(object,points,AOU,years,band,visits,output,...){
            ...
            DayMat<-getVisits(object=object,points=points,years=years,times=times,visits=visits)%>%
              dplyr::select(Admin_Unit_Code,Point_Name,EventDate,Visit,Year, Ord.Day) %>%
              mutate(Visit=paste0("Visit",Visit)) %>%
              group_by(Admin_Unit_Code,Point_Name, Year, Visit) %>%
              summarize(Day=max(Ord.Day)) %>%
              spread(key=Visit,value=Day,fill=0) 
              names(DayMat)[names(DayMat)=="Visit1"]<-"Day1"
              names(DayMat)[names(DayMat)=="Visit2"]<-"Day2"
            ...
            # if( ncol( DayMat)> 3) {
            #   DayMat[4:ncol(DayMat)]<-DayMat[4:ncol(DayMat)]*VisitMat[4:ncol(DayMat)]
            # }
           
            
            DayMat<-DayMat %>% ungroup  # to fix errors with dplyr when maniplating grouped tables
            return(DayMat)
