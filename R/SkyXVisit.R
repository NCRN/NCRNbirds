...
setGeneric(name="SkyXVisit",function(object,points=NA,AOU=NA,years=NA,times=NA,band=1,visits=NA,
                                       output="dataframe",...){standardGeneric("SkyXVisit")}, signature="object")



setMethod(f="SkyXVisit", signature=c(object="list"),
          function(object, points, AOU, years, band, visits, output,...) {
            OutMat<-lapply(X=object, FUN=SkyXVisit, points=points, AOU=AOU, years=years, times=times,band=band,visits=visits,...)
            switch(output,
                   list= return(OutMat),
...
setMethod(f="SkyXVisit", signature=c(object="NCRNbirds"),
          function(object,points,AOU,years,band,visits,output,...){
            ...
            SkyMat<-getVisits(object=object,points=points,years=years,times=times,visits=visits)%>%
              dplyr::select(Admin_Unit_Code,Point_Name,EventDate,Visit,Year, Sky_Condition) %>%
              mutate(Visit=paste0("Visit",Visit)) %>%
              group_by(Admin_Unit_Code,Point_Name, Year, Visit) %>%
              summarize(Sky=max(Sky_Condition)) %>%
              spread(key=Visit,value=Sky,fill=0) 
              names(SkyMat)[names(SkyMat)=="Visit1"]<-"Sky_Condition1"
              names(SkyMat)[names(SkyMat)=="Visit2"]<-"Sky_Condition2"
            ...
            # if( ncol( SkyMat)> 3) {
            #   SkyMat[4:ncol(SkyMat)]<-SkyMat[4:ncol(SkyMat)]*VisitMat[4:ncol(SkyMat)]
            # }
           
            
            SkyMat<-SkyMat %>% ungroup  # to fix errors with dplyr when maniplating grouped tables
            return(SkyMat)
