...
#' @title mapAbundance
#' 
#' @description Generates map of abundance estimates at the point-level that pool data among years and control for detection probability.
#' 
...
setGeneric(name="mapAbundance",function(object,points=NA,AOU=NA,years=NA,times=NA,band=1,visits=NA,mixture="P",site.covs="Point_Name",obs.covs=NA,Table=TRUE, Figure=TRUE,color=NA,APIkey=NA,...){standardGeneric("mapAbundance")}, signature="object")


############ need to add list method I

setMethod(f="mapAbundance", signature=c(object="list"),
          function(object,points,AOU,years,times,band,visits,mixture,site.covs,obs.covs,Table,Figure,APIkey,...) {
            
            return(lapply(X=object, FUN=mapAbundance, points=points,AOU=AOU,years=years,times=times,band=band,
...
setMethod(f="mapAbundance", signature=c(object="NCRNbirds"),
          function(object,points,AOU,years,times,band,visits,mixture,site.covs,obs.covs,Table,Figure,color,APIkey,...){
            ...
            #run abundance model
            mod<-nmixBirds_zl(object=parkData,AOU=AOU,years=years,times=times,band=band,visits=visits,mixture=mixture,site.covs=site.covs, obs.covs=obs.covs)
            ...
            mod.abun.predict.out.1<-unique(mod.abun.predict[,c("Admin_Unit_Code","Point_Name","Predicted","SE","lower","upper")])
            ...
            #reorder columns
            mod.abun.predict.out<-mod.abun.predict.out.1[,c("Common_Name","AOU_Code","Park_Name","Admin_Unit_Code","Point_Name","Predicted","SE","lower","upper")]
            
            #add coords
            point.data<-parkData@Points
            coords<-unique(point.data[,c("Point_Name","Latitude","Longitude")])
            mod.abun.predict.out<-merge(mod.abun.predict.out, coords, by="Point_Name",all.x=TRUE)
            ...
            #sort Abun.Table by highest to lowest abundance
            Abun.Table<-Abun.Table[order(Abun.Table$Predicted,decreasing=TRUE),]
            
            #reorder Point_Name factor levels
            Abun.Table$Point_Name<-factor(Abun.Table$Point_Name, levels=rev(unique(as.character(Abun.Table$Point_Name))))
            ...
            abunPlot<-ggplot(data=Abun.Table, aes(x=Point_Name, y=Predicted))+
              ...
              #scale_y_continuous(breaks=seq(0,round(max(Abun.Table$Predicted,na.rm=TRUE),0),length.out=5), expand=c(0.1,0.1), limits=c(-1,300))+
              #coord_cartesian(ylim=c(0,max(Abun.Table$Predicted,na.rm=TRUE)))+
              labs(x="Sampling Point Name", y="Estimated Abundance (birds per point)")+
              coord_flip()+
              ggtitle(paste(strwrap(paste("Estimated Abundance for the ", myBirdName, " in ", myParkName, " from ", min(years), " to ", max(years)), width=70),collapse = "\n"))
            #abunPlot
            
            
            #create map of abundance among points
            mean.x<-mean(Abun.Table$Longitude, na.rm=TRUE)
            mean.y<-mean(Abun.Table$Latitude, na.rm=TRUE)
            ...
            park.map.out <- ggmap(park.map, darken = c(0.2, "white"))+
              ...
              geom_point(data=Abun.Table, shape=21, aes(x=Longitude,y=Latitude, size=Predicted), fill=alpha(color,0.5), color=color)+
              ...
              theme(axis.text.x = element_text(size=10, color="black"),
                    ...
                    title=element_text(size=14))+
              labs(x="Longitude",y="Latitude", size=paste("Abundance"))+
              ggtitle(paste(strwrap(paste("Map of Point-Level Mean Abundance for the ", myBirdName, " in ", myParkName," from ", min(years), " to ", max(years)), width=70),collapse = "\n"))
            #park.map.out
            ...
            ifelse(response=="y",
                   {
                     ...
                     write.csv(Abun.Table, file=paste(paste("NCRNbirds_Output", Sys.Date(), sep="_"), paste(paste(myParkName, myBirdName, "AbundanceTableByPoint",Sys.Date(), sep="_"),".csv", sep=""),  sep="/"), row.names=FALSE)

                     ggsave(abunPlot, file=paste(paste("NCRNbirds_Output", Sys.Date(), sep="_"), paste(paste(myParkName, myBirdName, "AbundancePlotByPoint",Sys.Date(), sep="_"),".png", sep=""),sep="/"), width=10, height=10, units="in", dpi=300)

                     ggsave(park.map.out, file=paste(paste("NCRNbirds_Output", Sys.Date(), sep="_"), paste(paste(myParkName, myBirdName, "MapAbundance",Sys.Date(), sep="_"),".png", sep=""),sep="/"), width=10, height=10, units="in", dpi=300)


                     #return table and fig
                     #return table and fig
                     if(isTRUE(Table) & isFALSE(Figure)){
                       return(list(mod, Abun.Table, park.map.out))
                     }else{
                       if(isFALSE(Table) & isTRUE(Figure)){
                         return(list(mod, abunPlot, park.map.out))
                       }else{
                         if(isFALSE(Table) & isFALSE(Figure)){
                           return(mod)
                         }else{
                           return(list(mod, Abun.Table, abunPlot, park.map.out))
                         }}}
                   },
                   #return table and fig
                   if(isTRUE(Table) & isFALSE(Figure)){
