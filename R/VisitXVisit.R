...
setGeneric(name="VisitXVisit",function(object,points=NA,AOU=NA,years=NA,times=NA,band=1,visits=NA,
                                       output="dataframe",...){standardGeneric("VisitXVisit")}, signature="object")



setMethod(f="VisitXVisit", signature=c(object="list"),
          function(object, points, AOU, years, band, visits, output,...) {
            OutMat<-lapply(X=object, FUN=VisitXVisit, points=points, AOU=AOU, years=years, times=times,band=band,visits=visits,...)
            switch(output,
                   list= return(OutMat),
...
setMethod(f="VisitXVisit", signature=c(object="NCRNbirds"),
          function(object,points,AOU,years,band,visits,output,...){
            ...
            VisitMat$Visit1<-ifelse(is.na(VisitMat$Visit1), VisitMat$Visit1, 1)
            VisitMat$Visit2<-ifelse(is.na(VisitMat$Visit2), VisitMat$Visit2, 2)
            ...
            # if( ncol( VisitMat)> 3) {
            #   VisitMat[4:ncol(VisitMat)]<-VisitMat[4:ncol(VisitMat)]*VisitMat[4:ncol(VisitMat)]
            # }
           
            
            VisitMat<-VisitMat %>% ungroup  # to fix errors with dplyr when maniplating grouped tables
            return(VisitMat)
