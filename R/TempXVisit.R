...
setGeneric(name="TempXVisit",function(object,points=NA,AOU=NA,years=NA,times=NA,band=1,visits=NA,
                                       output="dataframe",...){standardGeneric("TempXVisit")}, signature="object")



setMethod(f="TempXVisit", signature=c(object="list"),
          function(object, points, AOU, years, band, visits, output,...) {
            OutMat<-lapply(X=object, FUN=TempXVisit, points=points, AOU=AOU, years=years, times=times,band=band,visits=visits,...)
            switch(output,
                   list= return(OutMat),
...
setMethod(f="TempXVisit", signature=c(object="NCRNbirds"),
          function(object,points,AOU,years,band,visits,output,...){
            ...
            TempMat<-getVisits(object=object,points=points,years=years,times=times,visits=visits)%>%
              dplyr::select(Admin_Unit_Code,Point_Name,EventDate,Visit,Year, Temperature) %>%
              mutate(Visit=paste0("Visit",Visit)) %>%
              group_by(Admin_Unit_Code,Point_Name, Year, Visit) %>%
              summarize(Temp=max(Temperature)) %>%
              spread(key=Visit,value=Temp,fill=0) 
              names(TempMat)[names(TempMat)=="Visit1"]<-"Temperature1"
              names(TempMat)[names(TempMat)=="Visit2"]<-"Temperature2"
            ...
            # if( ncol( TempMat)> 3) {
            #   TempMat[4:ncol(TempMat)]<-TempMat[4:ncol(TempMat)]*VisitMat[4:ncol(TempMat)]
            # }
           
            
            TempMat<-TempMat %>% ungroup  # to fix errors with dplyr when maniplating grouped tables
            return(TempMat)
