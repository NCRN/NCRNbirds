...
#' 
#' Several option exist for coloring the points based on the \code{colorNumeric}, \code{colorBin}, \code{colorQuantile}, and \code{colorFactor} functions in leaflet.
#' 
#' quantile - This option will divide the points into the equal quantiles based on the data in \code{values}. The \code{colorgroups} agrument indicates the number of groups the points will be divided into. For exmple if \code{colorgroups=4} then the points with the bottom quarter of \code{values} will get one color, the second quarter of \code{values} will get a second color etc. An erorr may occur if it is not possible to divide the \code{value} vector in to the indicate number of groups. For example, if half of \code{values} is the same number, than it cannot be divided into thirds.
#' 
#' bin - This option also divides the points into groups, but in this case the groups either cover an equal range in \code{values} or cover ranges specificed in \code{colorgroups}. If \code{colorgroups} is a single value, than that will indicate the number of groups to divide \code{values} into. For example, if \code{values} range from 1 to 100, and \code{colorgroups} is 4, the points with values 1-25 will be one color, 26-50 a second and so on. If instead \code{colorgroups} is \code{c(-1,10,50,75,101)} then points from 1-10 will be one color, 11-50 a second and so on. 
#' 
