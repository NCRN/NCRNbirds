...
#' @title HumidXVisit
#' 
#' @description Produces a Humidity X Visit matrix for use in analyses
#' 
...
setGeneric(name="HumidXVisit",function(object,points=NA,AOU=NA,years=NA,times=NA,band=1,visits=NA,
                                       output="dataframe",...){standardGeneric("HumidXVisit")}, signature="object")



setMethod(f="HumidXVisit", signature=c(object="list"),
          function(object, points, AOU, years, band, visits, output,...) {
            OutMat<-lapply(X=object, FUN=HumidXVisit, points=points, AOU=AOU, years=years, times=times,band=band,visits=visits,...)
            switch(output,
                   list= return(OutMat),
...
setMethod(f="HumidXVisit", signature=c(object="NCRNbirds"),
          function(object,points,AOU,years,band,visits,output,...){
            ...
            HumidMat<-getVisits(object=object,points=points,years=years,times=times,visits=visits)%>%
              dplyr::select(Admin_Unit_Code,Point_Name,EventDate,Visit,Year, Humidity) %>%
              mutate(Visit=paste0("Visit",Visit)) %>%
              group_by(Admin_Unit_Code,Point_Name, Year, Visit) %>%
              summarize(Humid=max(Humidity)) %>%
              spread(key=Visit,value=Humid,fill=0) 
              names(HumidMat)[names(HumidMat)=="Visit1"]<-"Humidity1"
              names(HumidMat)[names(HumidMat)=="Visit2"]<-"Humidity2"
            ...
            # if( ncol( HumidMat)> 3) {
            #   HumidMat[4:ncol(HumidMat)]<-HumidMat[4:ncol(HumidMat)]*VisitMat[4:ncol(HumidMat)]
            # }
           
            
            HumidMat<-HumidMat %>% ungroup  # to fix errors with dplyr when maniplating grouped tables
            return(HumidMat)
