#' @include NCRNbirds_Class_def.R getChecklist.R CountXVisit.R
#' 
#' @title SumRelAbund
#' 
...
#' @importFrom dplyr arrange group_by left_join mutate n select slice summarize ungroup
#' @importFrom magrittr %>%
#' @importFrom tidyr complete nesting gather
...
#' @param SortRelAbund Logical, if \code{TRUE}, when multiple species are selected it will calculate and sort relative abundance per species. See \code{abund}.
#' @param abund Numeric, When \code{SortRelAbund} = \code{TRUE}, used to provide a numeric value to select the most abundant species.
...
setMethod(f="SumRelAbund", signature=c(object="list"),
          function(object, points, AOU, years,times, band, visits, CalcByYear,max, SortRelAbund, abund, output,...) {
            OutMat<-lapply(X=object, FUN=SumRelAbund, points=points, AOU=AOU, years=years, 
                           times=times,band=band,visits=visits, CalcByYear=CalcByYear,max=max, SortRelAbund=SortRelAbund, abund=abund, output=output, ...)
            switch(output,
                   list= return(OutMat),
...
setMethod(f="SumRelAbund", signature=c(object="NCRNbirds"),
          function(object,points,AOU,years,band,visits,CalcByYear,max, SortRelAbund, abund, output, ...){
            ...
            data<-map_dfr(.x=set_names(BirdNames, nm=BirdNames),.id= "AOU_Code", ~CountXVisit(object,AOU= .x, points=points, visits=visits, years=years, band=band, ...)) %>% 
            ...
            {if(!CalcByYear)  dplyr::group_by(.,Admin_Unit_Code,AOU_Code,visit, Year) else # sum across all visits and years
            dplyr::group_by(., Admin_Unit_Code,AOU_Code, Year)} %>%  # sum across all visits by year
            dplyr::summarize_all(Total= sum(value, na.rm=TRUE), Mean= round(mean(value, na.rm=TRUE),digits=3), 
