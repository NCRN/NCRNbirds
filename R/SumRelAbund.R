...
#' @param CalcBy  If = \code{TRUE},will calculate mean detections across all visits per year. Defaults to calculating per visit.
...
#' @param abund When \code{SortRelAbund} = \code{TRUE} provide a numeric value to select the most abundant species.
...
#' 
#' @details Summaries relative abundance by species (avg detections per point) for a \code{NCRNbirds} object or a \code{list} of such objects. 
#'  
...
setMethod(f="SumRelAbund", signature=c(object="list"),
          function(object, points, AOU, years, band, visits, CalcByYear,SortRelAbund,abund, output,...) {
            OutMat<-lapply(X=object, FUN=SumRelAbund, points=points, AOU=AOU, years=years, 
                           times=times,band=band,visits=visits, CalcByYear=CalcByYear,SortRelAbund=SortRelAbund, abund=abund, output, ...)
            switch(output,
                   list= return(OutMat),
