...
#' @param CombineData  A logical (default is FALSE) to combine data from multiple park units, or entire Network
...
getParkData<-function(Dir,UnitCode,CombineData=FALSE){
  ...
  NCRNnames.df<-data.frame(Network="NCRN",Admin_Unit_Code="NCRN",ShortName ="Nat. Cap. Region",LongName="National Capital Region Network")
  
  ParkNames.df<-rbind(NCRNnames.df,unique(InPoints[,c("Network","Admin_Unit_Code","ShortName","LongName")]))
  
  #convert factor to character
  ParkNames.df$Network<-as.character(ParkNames.df$Network)
  ParkNames.df$Admin_Unit_Code<-as.character(ParkNames.df$Admin_Unit_Code)
  ParkNames.df$ShortName<-as.character(ParkNames.df$ShortName)
  ParkNames.df$LongName<-as.character(ParkNames.df$LongName)
  
    #any user-desired combination of parks
    ParkNames.sub<-subset(ParkNames.df, Admin_Unit_Code %in% UnitCode)
    
    
    if(CombineData){
      COMB<-new("NCRNbirds",
                ParkCode=ParkNames.sub$Admin_Unit_Code,
                ShortName=ParkNames.sub$ShortName,
                LongName=ParkNames.sub$LongName, 
                Network=ParkNames.sub$Network,
      ...
      COMB<-new("NCRNbirds",
                ...
                Points=InPoints[InPoints$Admin_Unit_Code %in% UnitCode,], 
                Visits=InVisits[InVisits$Admin_Unit_Code %in% UnitCode,],
                Birds=InFieldData[InFieldData$Admin_Unit_Code %in% UnitCode,],
                Species=InSpecies
      )
